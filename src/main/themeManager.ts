...
import {fluentThemeSource} from 'common/fluentTheme';
...
export class ThemeManager {
    ...
    private updateMainViews = () => {
        if (this.applyFluentThemeSource()) {
            return;
        }
    ...
    private applyFluentThemeSource = () => {
        if (Config.fluentEnabled) {
            const source = fluentThemeSource(Config.fluentTheme);

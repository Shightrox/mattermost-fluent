...
import {
    ...
    FLUENT_HISTORY_STATUS,
    BROWSER_HISTORY_STATUS_UPDATED,
} from 'common/communication';
import {Logger} from 'common/log';
import ServerManager from 'common/servers/serverManager';
import {fluentToolbarCommandSchema, ipcValidate} from 'common/Validator';
...
export class TabManager extends EventEmitter {
    ...
    constructor() {
        ...
        ipcMain.handle(FLUENT_HISTORY_STATUS, (event) => {
            if (event.sender !== MainWindow.get()?.webContents) {
                return {canGoBack: false, canGoForward: false};
            }
            return this.getCurrentActiveTabView()?.getBrowserHistoryStatus() ?? {canGoBack: false, canGoForward: false};
        });
        ipcMain.on(FLUENT_TOOLBAR_COMMAND, ipcValidate((event: IpcMainEvent, command: 'search' | 'tools' | 'back' | 'forward') => {
            // Only the trusted main-window toolbar can command the active tab.
            if (event.sender !== MainWindow.get()?.webContents) {
                return;
            }
            ...
            if (view && !view.isDestroyed()) {
                ...
                if (command === 'back' || command === 'forward') {
                    view.goToOffset(command === 'back' ? -1 : 1);
                    return;
                }
                view.getWebContentsView().webContents.send(FLUENT_TOOLBAR_COMMAND, command);
            }
        }, [fluentToolbarCommandSchema]));
    ...
    private setupTab = (view: MattermostView, webContentsView: MattermostWebContentsView) => {
        ...
        const updateHistory = (canGoBack: boolean, canGoForward: boolean) => {
            if (this.isActiveTab(webContentsView.id)) {
                mainWindow.sendToRenderer(FLUENT_HISTORY_STATUS, {canGoBack, canGoForward});
            }
        };
        webContentsView.on(BROWSER_HISTORY_STATUS_UPDATED, updateHistory);
        ...
        this.tabListeners.set(webContentsView.id, () => {
            ...
            webContentsView.off(BROWSER_HISTORY_STATUS_UPDATED, updateHistory);

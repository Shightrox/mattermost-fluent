...
describe('TabManager', () => {
    ...
    it('only forwards allowed toolbar commands from the trusted main renderer', () => {
        const manager = new TabManager();
        const trusted = {};
        const send = jest.fn();
        const focus = jest.fn();
        const goToOffset = jest.fn();
        MainWindow.get.mockReturnValue({webContents: trusted});
        jest.spyOn(manager, 'getCurrentActiveTabView').mockReturnValue({
            isDestroyed: () => false,
            focus,
            goToOffset,
            getWebContentsView: () => ({webContents: {send}}),
        });
        const handler = ipcMain.listeners(FLUENT_TOOLBAR_COMMAND).at(-1);
        handler({sender: {}}, 'search');
        handler({sender: trusted}, 'arbitrary-command');
        expect(send).not.toHaveBeenCalled();
        handler({sender: trusted}, 'search');
        expect(focus).toHaveBeenCalledTimes(1);
        expect(send).toHaveBeenCalledWith(FLUENT_TOOLBAR_COMMAND, 'search');
        handler({sender: {}}, 'back');
        expect(goToOffset).not.toHaveBeenCalled();
        handler({sender: trusted}, 'back');
        handler({sender: trusted}, 'forward');

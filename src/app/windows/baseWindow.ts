...
export default class BaseWindow {
    ...
    constructor(options: BrowserWindowConstructorOptions) {
        ...
        this.updateFluentMaterial();
        nativeTheme.on('updated', this.updateFluentMaterial);
    ...
    private onClosed = () => {
        nativeTheme.off('updated', this.updateFluentMaterial);
    ...
    private updateFluentMaterial = () => {
        if (this.win.isDestroyed()) {
            return;
        }
        const {enabled, material, theme} = getFluentSettings();
        try {
            if (process.platform === 'win32') {
                this.win.setBackgroundMaterial(material === 'solid' ? 'none' : material);
            }
            let background = '#000000';
            if (enabled) {
                background = theme === 'dark' ? '#202020' : '#f0f3f8';
                if (material !== 'solid') {
                    background = '#00000000';
                }
            }
            this.win.setBackgroundColor(background);
        } catch (error) {
            log.warn('Fluent material unavailable; using a solid background', error);
            this.win.setBackgroundColor(theme === 'dark' ? '#202020' : '#f0f3f8');
        }
    };

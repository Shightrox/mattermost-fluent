...
import {type WebContents, ipcMain, type BrowserWindow, WebContentsView, app, type IpcMainEvent} from 'electron';

import {LOADING_SCREEN_ANIMATION_FINISHED, TOGGLE_LOADING_SCREEN_VISIBILITY} from 'common/communication';
...
export class LoadingScreen {
    ...
    fade = () => {
        if (this.state === LoadingScreenState.VISIBLE) {
            ...
            // A renderer error or disabled CSS transition must never leave an
            // invisible native view intercepting the ready application's mouse.
            this.fadeTimeout = setTimeout(() => {
                if (this.state === LoadingScreenState.FADING && !this.parent.isDestroyed()) {
    ...
    destroy = () => {
        clearTimeout(this.fadeTimeout);
        this.parent.contentView.off('bounds-changed', this.setBounds);
    ...
    registerThemeManager = (register: (webContents: WebContents) => void) => {
        register(this.view.webContents);
    };

    private handleAnimationFinished = (event?: IpcMainEvent) => {
        if ((event && event.sender !== this.view.webContents) || this.state !== LoadingScreenState.FADING) {
            return;
        }
        clearTimeout(this.fadeTimeout);

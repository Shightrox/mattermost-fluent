...
export class Config extends EventEmitter {
    ...
    get fluentEnabled() {
        return this.combinedData?.fluentEnabled ?? true;
    }

    get fluentMaterial() {
        return this.combinedData?.fluentMaterial ?? 'mica';
    }

    get fluentChatOpacity() {
        return this.combinedData?.fluentChatOpacity ?? 75;
    }

    get fluentMotion() {
        return this.combinedData?.fluentMotion ?? true;
    }

    get fluentTint() {
        return this.combinedData?.fluentTint ?? 'neutral';
    }

    get fluentAccent() {
        return this.combinedData?.fluentAccent ?? 'blue';
    }

    get fluentTextSize() {
        return this.combinedData?.fluentTextSize ?? 'standard';
    }

    get fluentTheme() {
        const theme = this.combinedData?.fluentTheme;
        return !theme || theme === 'server' ? 'dark' : theme;
    }
    ...
    private regenerateCombinedConfigData = () => {
        ...
        if (this.combinedData) {
            ...
            if (this.combinedData.fluentTheme === 'server') {
                this.combinedData.fluentTheme = 'dark';
            }
        }
    };
}

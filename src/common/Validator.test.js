...
describe('common/Validator', () => {
    ...
    describe('validateV4ConfigData', () => {
        const config = {
            ...
            fluentEnabled: true,
            fluentMaterial: 'mica',
            fluentChatOpacity: 75,
            fluentMotion: true,
            fluentAccent: 'blue',
            fluentTint: 'neutral',
            fluentTextSize: 'standard',
            fluentTheme: 'dark',
            enableMetrics: true,
            enableSentry: true,
        ...
        it('defaults old Fluent profiles and rejects unsupported appearance values', () => {
            const oldProfile = {...config};
            delete oldProfile.fluentAccent;
            delete oldProfile.fluentTint;
            delete oldProfile.fluentTextSize;
            delete oldProfile.fluentChatOpacity;
            expect(Validator.validateV4ConfigData(oldProfile)).toMatchObject({fluentAccent: 'blue', fluentTextSize: 'standard'});
            expect(Validator.validateV4ConfigData(oldProfile).fluentTint).toBe('neutral');
            expect(Validator.validateV4ConfigData({...config, fluentTint: 'url(https://example.com)'})).toBeNull();
            for (const fluentTint of ['neutral', 'slate', 'ocean', 'forest', 'plum', 'sand']) {
                expect(Validator.validateV4ConfigData({...config, fluentTint}).fluentTint).toBe(fluentTint);
            }
            for (const fluentAccent of ['teal', 'rose', 'amber', 'coral', 'indigo']) {
                expect(Validator.validateV4ConfigData({...config, fluentAccent}).fluentAccent).toBe(fluentAccent);
            }
            expect(Validator.validateV4ConfigData({...config, fluentAccent: 'url(https://example.com)'})).toBeNull();
            expect(Validator.validateV4ConfigData({...config, fluentTextSize: 9000})).toBeNull();
            expect(Validator.validateV4ConfigData(oldProfile).fluentChatOpacity).toBe(75);
            for (const fluentChatOpacity of [-1, 101, 50.5, 'url(https://example.com)']) {
                expect(Validator.validateV4ConfigData({...config, fluentChatOpacity})).toBeNull();
            }
        });

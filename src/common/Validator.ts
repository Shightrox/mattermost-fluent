...
const configDataSchemaV4 = Joi.object<ConfigV4>({
    ...
    fluentEnabled: Joi.boolean().default(true),
    fluentMaterial: Joi.string().valid('solid', 'mica', 'acrylic').default('mica'),
    fluentChatOpacity: Joi.number().integer().min(30).max(100).default(75),
    fluentMotion: Joi.boolean().default(true),
    fluentAccent: Joi.string().valid('blue', 'violet', 'green', 'teal', 'rose', 'amber', 'coral', 'indigo').default('blue'),
    fluentTint: Joi.string().valid('neutral', 'slate', 'ocean', 'forest', 'plum', 'sand').default('neutral'),
    fluentTextSize: Joi.string().valid('standard', 'large').default('standard'),
    fluentTheme: Joi.string().valid('system', 'light', 'dark', 'server').default('dark'),
...
export const fluentToolbarCommandSchema = Joi.string().valid('search', 'tools', 'back', 'forward').required();
export const fluentWorkspaceIdSchema = Joi.string().min(1).max(256).required();
